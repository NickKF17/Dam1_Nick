...
public class Equipo {
	...
	public void anyadeParticipante(Participante p) {
		if (p.getNacionalidad().equals(this.nacionalidad)==false)
			System.out.printf("El jugador %s no puede participar en el equipo de %s de %s porque su nacionalidad es %s",p.getNombre(),this.deporte.getNombre(),this.nacionalidad,p.getNacionalidad());
		else
