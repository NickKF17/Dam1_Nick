...
\newpage
\section*{Introducción} En SQL, una \textbf{subconsulta} es una consulta anidada dentro de otra consulta más grande.
%
...
%
Nos enfocaremos en un enfoque pedagógico, empezando por subconsultas sencillas en la cláusula \texttt{WHERE}, pasando por subconsultas correlacionadas (que dependen de la fila externa) y subconsultas en la cláusula \texttt{FROM} (tablas derivadas) con agregación.
%
Asumimos que el lector ya sabe usar consultas básicas \texttt{SELECT} (incluso con \texttt{JOIN} y agrupamientos simples), pero no ha utilizado vistas, procedimientos almacenados ni expresiones más avanzadas.
%
Añadiremos explicaciones detalladas y comentarios en el código SQL para aclarar cada ejemplo.
%
Todos los ejemplos han sido comprobados en el esquema oficial de \texttt{Sakila} y devuelven los resultados esperados. 

\vspace{1ex} % small vertical space

\textbf{Convención:} Para mayor legibilidad, en los listados usaremos mayúsculas para palabras clave SQL (\texttt{SELECT}, \texttt{WHERE}, etc.), y comentarios precedidos de \texttt{--} para explicar partes clave de las consultas. \section{Subconsultas en la cláusula WHERE (no correlacionadas)} Comenzamos con subconsultas \emph{no correlacionadas} en la cláusula \texttt{WHERE}.
%
Una subconsulta no correlacionada es aquella que la consulta externa puede ejecutar independientemente, porque no depende de cada fila externa.
%
...
%
Otro ejemplo clásico es comparar contra un valor agregado.
%
...
\subsection*{Uso de IN para subconsultas que devuelven conjuntos} Si la subconsulta puede devolver múltiples filas (por ejemplo, una lista de \texttt{id}s), debemos usar un operador de conjunto como \texttt{IN} (o su negación \texttt{NOT IN}) en lugar de un operador de comparación simple.
%
...
%
Por ejemplo, supongamos que queremos listar las películas cuya duración es \textbf{mayor que la de cualquier película} de la categoría \texttt{Sports}.
%
...
\end{itemize} El resultado será la lista de títulos de las películas nunca rentadas.
%
...
\textbf{Camino de tablas:} \texttt{film} $\rightarrow$ (anti-camino) \texttt{inventory} $\rightarrow$ \texttt{rental}.
...
\end{tabular}
\end{center} Este enfoque de subconsulta escalar en el \texttt{SELECT} es equivalente a hacer un \texttt{JOIN} con \texttt{rental} e \texttt{inventory} seguido de un \texttt{GROUP BY film}, pero a veces la subconsulta resulta más sencilla de entender por filas: “para esta película, calcula este dato agregado”.
%
...
%
Al poner una subconsulta en el \texttt{FROM}, es obligatorio asignarle un alias, igual que si fuera una tabla (porque la consulta externa debe referirse a ese resultado).
%
...
%
Sin embargo, dado que nos limitamos a lo que los estudiantes conocen (SQL básico sin CTEs) y queremos mantener la claridad, hemos optado por la solución mostrada.
%
...
%
Al hacer un \texttt{JOIN} sin especificar condición (\texttt{JOIN} implícitamente es un \texttt{CROSS JOIN} cuando no hay \texttt{ON}), esa única fila se “adhiere” a cada película \texttt{f}, permitiéndonos comparar \texttt{f.length} con \texttt{stats.avg\_length} fácilmente.
%
...
\textbf{Camino:} \texttt{film} $\rightarrow$ \texttt{film\_category} $\rightarrow$ \texttt{category} y anti-camino \texttt{inventory} $\rightarrow$ \texttt{rental}. \\
...
% ===================== 10 =====================
\subsection{Correlacionada (NOT EXISTS) — Idiomas de language sin películas}
...
% ===================== 14 =====================
\subsection{Escalar — ¿Cuántos idiomas hay en language?}
\textbf{Camino:} \texttt{language}
...
% ===================== 15 =====================
\subsection{Correlacionada (EXISTS) — Idiomas con al menos una peli}
