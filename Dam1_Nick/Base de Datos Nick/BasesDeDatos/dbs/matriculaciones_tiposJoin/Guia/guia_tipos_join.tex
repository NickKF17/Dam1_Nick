...
%
Te dejo como reto que construyas las consultas de \texttt{left join} utilizando un \textt{right join} y viceversa.
...
% =========================
\section{Tipo test (Punto 5, sin soluciones marcadas)}
% =========================
...
\item INNER JOIN devuelve…
  \begin{enumerate}[label=\Alph*.]
    \item Todas las filas de la izquierda.
...
\item En un RIGHT JOIN, ¿qué se garantiza?
  \begin{enumerate}[label=\Alph*.]
    \item Ver todas las filas de la izquierda.
    \item Ver todas las filas de la derecha.
...
\item Si haces INNER JOIN \texttt{matriculas m} con \texttt{cursos c} ON \texttt{m.id\_curso=c.id\_curso}, ¿aparece la matrícula 1005?
  \begin{enumerate}[label=\Alph*.]
...
\item Para listar todos los cursos y cuántos alumnos tienen (incluye 0), lo más correcto es…
  \begin{enumerate}[label=\Alph*.]
    ...
    \item LEFT JOIN desde \texttt{cursos} hacia \texttt{matriculas} y \texttt{GROUP BY}.
...
\subsection*{Soluciones de los ejercicios (Punto 4)}
...
\subsection*{Plantilla de corrección del tipo test (Punto 5)}
\begin{center}
